...
#include "animaDiffusionEstimateODFImageFilter.h"
...
namespace anima
{
    ...
    void
    DiffusionEstimateODFImageFilter<TInputPixelType, TOutputPixelType>::AddGradientDirection(unsigned int i, std::vector<double> &grad)
    {
        if (isZero(grad))
        {
    ...
    template <typename TInputPixelType, typename TOutputPixelType>
    void
    DiffusionEstimateODFImageFilter<TInputPixelType, TOutputPixelType>::GenerateOutputInformation()
    {
    ...
    template <typename TInputPixelType, typename TOutputPixelType>
    void
    DiffusionEstimateODFImageFilter<TInputPixelType, TOutputPixelType>::BeforeThreadedGenerateData()
    {
        unsigned int vectorLength = (m_LOrder + 1) * (m_LOrder + 2) / 2;
    ...
    template <typename TInputPixelType, typename TOutputPixelType>
    void
    DiffusionEstimateODFImageFilter<TInputPixelType, TOutputPixelType>::DynamicThreadedGenerateData(const OutputImageRegionType &outputRegionForThread)
    {
        ...
        using OutputScalarIteratorType = itk::ImageRegionIterator<OutputScalarImageType>;
