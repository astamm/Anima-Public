...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
bool MCMPairingMeanSquaresImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
typename MCMPairingMeanSquaresImageToImageMetric<
    ...
    if (this->m_Interpolator->IsInsideBuffer(transformedIndex)) {
      ...
      if (!isZero(movingValue)) {
        ...
        if (tensorCompatibilityCondition)
          ...
          measure +=
              this->ComputeNonTensorBasedMetricPart(i, currentMovingValue);
      } else {
        if (tensorCompatibilityCondition)
          measure +=
              this->ComputeTensorBasedMetricPart(i, m_ZeroDiffusionModel);
        else
          measure +=
              this->ComputeNonTensorBasedMetricPart(i, m_ZeroDiffusionModel);
      }
    } else {
      if (tensorCompatibilityCondition)
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
double
MCMPairingMeanSquaresImageToImageMetric<TFixedImagePixelType,
                                        TMovingImagePixelType, ImageDimension>::
    ComputeTensorBasedMetricPart(unsigned int index,
                                 const MCModelPointer &movingValue) const {
  unsigned int fixedNumCompartments =
      m_FixedImageValues[index]->GetNumberOfCompartments();
  ...
  for (unsigned int i = 0; i < fixedNumCompartments; ++i) {
    ...
    m_leCalculator->GetTensorLogarithm(m_FixedImageValues[index]
                                           ->GetCompartment(i)
                                           ->GetDiffusionTensor()
                                           .GetVnlMatrix()
                                           .as_matrix(),
                                       workLogMatrix);
    anima::GetVectorRepresentation(workLogMatrix, fixedLogVectors[pos], 6,
                                   true);
  ...
  for (unsigned int i = 0; i < movingNumCompartments; ++i) {
    ...
    m_leCalculator->GetTensorLogarithm(movingValue->GetCompartment(i)
                                           ->GetDiffusionTensor()
                                           .GetVnlMatrix()
                                           .as_matrix(),
                                       workLogMatrix);
    anima::GetVectorRepresentation(workLogMatrix, movingLogVectors[pos], 6,
                                   true);
  ...
  if (!m_OneToOneMapping)
    totalNumPairingVectors =
        boost::math::factorial<double>(maxCompartmentsNumber - 1) /
        (boost::math::factorial<double>(minCompartmentsNumber - 1) *
         boost::math::factorial<double>(maxCompartmentsNumber -
                                        minCompartmentsNumber));

  std::vector<std::vector<unsigned int>> numPairingsVectors(
      totalNumPairingVectors);

  if (!m_OneToOneMapping) {
    std::vector<unsigned int> initialPairingsNumber(maxCompartmentsNumber - 1,
                                                    0);
    std::vector<unsigned int> pairing(minCompartmentsNumber, 1);
    for (unsigned int i = minCompartmentsNumber - 1;
         i < maxCompartmentsNumber - 1; ++i)
    ...
    do {
      ...
      ++countVector;
    } while (std::next_permutation(initialPairingsNumber.begin(),
                                   initialPairingsNumber.end()));
  } else {
  ...
  for (unsigned int l = 0; l < totalNumPairingVectors; ++l) {
    ...
    do {
      ...
      for (unsigned int j = 0; j < maxCompartmentsNumber; ++j) {
        ...
        if ((firstIndex >= fixedLogVectors.size()) ||
            (secondIndex >= movingLogVectors.size()))
          continue;

        double dist = 0;
        for (unsigned int k = 0; k < fixedLogVectors[firstIndex].GetSize(); ++k)
          dist += (fixedLogVectors[firstIndex][k] -
                   movingLogVectors[secondIndex][k]) *
                  (fixedLogVectors[firstIndex][k] -
                   movingLogVectors[secondIndex][k]);
        ...
        metricValue +=
            fixedWeights[firstIndex] * movingWeights[secondIndex] * dist;
      }
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
void MCMPairingMeanSquaresImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
