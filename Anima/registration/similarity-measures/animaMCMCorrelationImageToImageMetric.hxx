...
MCMCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
    ImageDimension>::MCMCorrelationImageToImageMetric() {
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
bool MCMCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
    ImageDimension>::CheckTensorCompatibility() const {
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
typename MCMCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType, ImageDimension>::MeasureType
MCMCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
    ImageDimension>::GetValue(const TransformParametersType &parameters) const {
  ...
  for (unsigned int i = 0; i < this->m_NumberOfPixelsCounted; ++i) {
    ...
    if (this->m_Interpolator->IsInsideBuffer(transformedIndex)) {
      ...
      if (!isZero(movingValue)) {
        MCModelPointer currentMovingValue =
            movingImage->GetDescriptionModel()->Clone();
        currentMovingValue->SetModelVector(movingValue);

        if (this->GetModelRotation() != Superclass::NONE)
          currentMovingValue->Reorient(
              this->m_OrientationMatrix,
              (this->GetModelRotation() == Superclass::PPD));
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
double MCMCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
    ImageDimension>::ComputeTensorBasedMetric(const std::vector<MCModelPointer>
                                                  &movingValues) const {
  typedef anima::MultiTensorSmoothingCostFunction CostFunctionType;
  typename CostFunctionType::Pointer smootherCostFunction =
      CostFunctionType::New();
  ...
  OptimizerType::ParametersType p(
      smootherCostFunction->GetNumberOfParameters());
  OptimizerType::ParametersType lowerBounds(
      smootherCostFunction->GetNumberOfParameters());
  OptimizerType::ParametersType upperBounds(
      smootherCostFunction->GetNumberOfParameters());

  lowerBounds[0] = m_LowerBoundGaussianSigma;
  upperBounds[0] = m_UpperBoundGaussianSigma;

  p[0] = m_LowerBoundGaussianSigma +
         (m_UpperBoundGaussianSigma - m_LowerBoundGaussianSigma) / 10.0;
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
double MCMCorrelationImageToImageMetric<TFixedImagePixelType,
                                        TMovingImagePixelType, ImageDimension>::
  ...
  smootherCostFunction->SetReferenceModels(m_FixedImageValues,
                                           m_GradientDirections, m_SmallDelta,
                                           m_BigDelta, m_GradientStrengths);
  smootherCostFunction->SetMovingModels(movingValues, m_GradientDirections,
                                        m_SmallDelta, m_BigDelta,
                                        m_GradientStrengths);
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
void MCMCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
    ImageDimension>::SetSmallDelta(double val) {
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
void MCMCorrelationImageToImageMetric<TFixedImagePixelType,
                                      TMovingImagePixelType,
                                      ImageDimension>::SetBigDelta(double val) {
...
void MCMCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
    ImageDimension>::SetGradientStrengths(std::vector<double> &val) {
  m_GradientStrengths = val;

  if ((m_GradientDirections.size() == m_GradientStrengths.size()) &&
      (m_GradientStrengths.size() != 0) && (m_GradientDirections.size() != 0))
    this->UpdateSphereWeights();
}
...
void MCMCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
    ImageDimension>::SetGradientDirections(std::vector<GradientType> &val) {
...
void MCMCorrelationImageToImageMetric<TFixedImagePixelType,
  ...
  std::sort(individualGradientStrengths.begin(),
            individualGradientStrengths.end());
  ...
  double lastBValue = anima::GetBValueFromAcquisitionParameters(
      m_SmallDelta, m_BigDelta,
      individualGradientStrengths[individualGradientStrengths.size() - 1]);
  ...
  for (unsigned int i = 0; i < individualGradientStrengths.size(); ++i) {
    ...
    double baseValue = 0;
    double bValueCenter = anima::GetBValueFromAcquisitionParameters(
        m_SmallDelta, m_BigDelta, individualGradientStrengths[i]);

    if (i > 0) {
      double bValueBefore = anima::GetBValueFromAcquisitionParameters(
          m_SmallDelta, m_BigDelta, individualGradientStrengths[i - 1]);
    ...
    if (i < individualGradientStrengths.size() - 1) {
      double bValueAfter = anima::GetBValueFromAcquisitionParameters(
          m_SmallDelta, m_BigDelta, individualGradientStrengths[i + 1]);
    ...
    m_SphereWeights[i] =
        4 * M_PI * (std::pow(upperRadius, 3.0) - std::pow(lowerRadius, 3.0)) /
        (3.0 * numValues);
  }
}
...
bool MCMCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
    ImageDimension>::isZero(PixelType &vector) const {
