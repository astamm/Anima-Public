...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
bool MTPairingCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
  ...
  MovingImageType *movingImage =
      const_cast<MovingImageType *>(this->GetFixedImage());
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
typename MTPairingCorrelationImageToImageMetric<
...
MTPairingCorrelationImageToImageMetric<
  ...
  std::vector<std::vector<double>> movingImageCompartmentWeights(
      this->m_NumberOfPixelsCounted);
  std::vector<std::vector<PixelType>> movingImageLogTensors(
      this->m_NumberOfPixelsCounted);
    ...
    if (this->m_Interpolator->IsInsideBuffer(transformedIndex)) {
      ...
      if (!isZero(movingValue)) {
        ...
        for (unsigned int j = 0;
             j < currentMovingValue->GetNumberOfCompartments(); ++j) {
          if (tmpWeights[internalCounter] <= 0)
            tmpWeights.erase(tmpWeights.begin() + internalCounter);
          else {
            ++internalCounter;
            m_leCalculator->GetTensorLogarithm(
                currentMovingValue->GetCompartment(j)
                    ->GetDiffusionTensor()
                    .GetVnlMatrix()
                    .as_matrix(),
                workLogMatrix);
        ...
        anima::GetVectorRepresentation(m_ZeroDiffusionModel->GetCompartment(0)
                                           ->GetDiffusionTensor()
                                           .GetVnlMatrix()
                                           .as_matrix(),
                                       movingImageLogTensors[i][0], 6, true);
  ...
  double mRS = this->ComputeMapping(
      m_FixedImageCompartmentWeights, m_FixedImageLogTensors,
      movingImageCompartmentWeights, movingImageLogTensors);
  double mRR = this->ComputeMapping(
      m_FixedImageCompartmentWeights, m_FixedImageLogTensors,
      m_FixedImageCompartmentWeights, m_FixedImageLogTensors);
  double mSS = this->ComputeMapping(
      movingImageCompartmentWeights, movingImageLogTensors,
      movingImageCompartmentWeights, movingImageLogTensors);
  double mRT = 0;
  double mST = 0;
  double numMaxCompartments =
      std::max(movingImage->GetDescriptionModel()->GetNumberOfCompartments(),
               m_NumberOfFixedCompartments);
  double epsilon =
      std::sqrt(numMaxCompartments / (3.0 * this->m_NumberOfPixelsCounted)) /
      numMaxCompartments;

  for (unsigned int i = 0; i < this->m_NumberOfPixelsCounted; ++i) {
    for (unsigned int k = 0; k < m_FixedImageCompartmentWeights[i].size();
         ++k) {
      double dotProd = 0;
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
double
MTPairingCorrelationImageToImageMetric<TFixedImagePixelType,
  ...
  for (unsigned int i = 0; i < this->m_NumberOfPixelsCounted; ++i) {
    ...
    unsigned int movingNumCompartments =
        movingImageCompartmentWeights[i].size();
    ...
    do {
      ...
      for (unsigned int j = 0; j < maxCompartmentsNumber; ++j) {
        ...
        if ((firstIndex >= refImageCompartmentWeights[i].size()) ||
            (secondIndex >= movingImageCompartmentWeights[i].size()))
          continue;

        double dist = 0;
        for (unsigned int k = 0;
             k < refImageLogTensors[i][firstIndex].GetSize(); ++k)
          dist += refImageLogTensors[i][firstIndex][k] *
                  movingImageLogTensors[i][secondIndex][k];

        distValue += refImageCompartmentWeights[i][firstIndex] *
                     movingImageCompartmentWeights[i][secondIndex] * dist;
      }

      if (std::abs(distValue) > std::abs(bestValue))
        bestValue = distValue;

    } while (std::next_permutation(currentPermutation.begin(),
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
void MTPairingCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
    ...
    if (!isZero(fixedValue)) {
      ...
      for (unsigned int i = 0; i < fixedMCM->GetNumberOfCompartments(); ++i) {
        if (tmpWeights[internalCounter] <= 0)
          ...
          ++internalCounter;
          m_leCalculator->GetTensorLogarithm(fixedMCM->GetCompartment(i)
                                                 ->GetDiffusionTensor()
                                                 .GetVnlMatrix()
                                                 .as_matrix(),
                                             workLogMatrix);
      ...
      anima::GetVectorRepresentation(m_ZeroDiffusionModel->GetCompartment(0)
                                         ...
                                         .GetVnlMatrix()
                                         .as_matrix(),
                                     m_FixedImageLogTensors[pos][0], 6, true);
