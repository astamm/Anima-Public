...
class MCMEstimatorImageFilter
    : public anima::MaskedImageToImageFilter<
          itk::Image<InputPixelType, 3>, anima::MCMImage<OutputPixelType, 3>> {
public:
  ...
  //! Denotes noise type on input signal. Based on this, a different cost
  ...
  void SetGradientStrengths(std::vector<double> &mb) {
    m_GradientStrengths = mb;
  }
  itkSetMacro(SmallDelta, double);
  itkSetMacro(BigDelta, double) void AddGradientDirection(unsigned int i,
                                                          GradientType &grad);
...
protected:
  ...
  CreateCostFunction(std::vector<double> &observedSignals,
                     MCMPointer &mcmModel);

  //! Create an optimizer following the optimizer type and estimation mode
  OptimizerPointer CreateOptimizer(CostFunctionBasePointer &cost,
                                   itk::Array<double> &lowerBounds,
                                   itk::Array<double> &upperBounds);

  //! Specific method for N=0 compartments estimation (only free water)
  void EstimateFreeWaterModel(MCMPointer &mcmValue,
                              std::vector<double> &observedSignals,
                              itk::ThreadIdType threadId, double &aiccValue,
                              double &b0Value, double &sigmaSqValue);

  //! Doing estimation of non isotropic compartments (for a given number of
  //! anisotropic compartments)
  void OptimizeNonIsotropicCompartments(
      MCMPointer &mcmValue, unsigned int currentNumberOfCompartments,
  ...
  void InitialOrientationsEstimation(MCMPointer &mcmValue,
                                     bool authorizedNegativeB0Value,
                                     unsigned int currentNumberOfCompartments,
                                     std::vector<double> &observedSignals,
                                     itk::ThreadIdType threadId,
                                     double &aiccValue, double &b0Value,
                                     double &sigmaSqValue);

  //! Doing estimation, calling initialization procedure until ball and
  //! zeppelin, returns AICc value
  void ModelEstimation(MCMPointer &mcmValue, bool authorizedNegativeB0Value,
                       std::vector<double> &observedSignals,
                       itk::ThreadIdType threadId, double &aiccValue,
                       double &b0Value, double &sigmaSqValue);

  //! Performs an optimization of the supplied cost function and parameters
  //! using the specified optimizer(s). Returns the optimized parameters.
  double PerformSingleOptimization(ParametersType &p,
                                   CostFunctionBasePointer &cost,
                                   itk::Array<double> &lowerBounds,
                                   itk::Array<double> &upperBounds);

  //! Performs initialization from single DTI
  virtual void SparseInitializeSticks(MCMPointer &complexModel,
                                      bool authorizeNegativeB0Value,
                                      std::vector<double> &observedSignals,
                                      itk::ThreadIdType threadId);

  //! Performs initialization from simplified model with the same number of
  //! compartments
  virtual void InitializeModelFromSimplifiedOne(MCMPointer &simplifiedModel,
  ...
  //! Utility function to get profiled data from the cost function into an MCM
  //! model
  void GetProfiledInformation(CostFunctionBasePointer &cost, MCMPointer &mcm,
                              double &b0Value, double &sigmaSqValue);

  //! Compute AICc value from a cost function value and model
  double ComputeAICcValue(MCMPointer &mcmValue, double costValue);

  //! Computes sphere radius coarse grid (used for sphere compartment
  //! initialization)
  void ComputeSphereRadiusCoarseGrid();

  //! Computes extra axonal and kappa coarse grids (used for NODDI and DDI
  //! initialization)
  void ComputeExtraAxonalAndKappaCoarseGrids();

  //! Computes extra axonal and tissue radius coarse grids (used for CHARMED
  //! initialization)
  void ComputeCHARMEDCoarseGrids();

  //! Computes extra axonal and kappa coarse grids (used for tensor final
  //! initaialization)
  void ComputeTensorRadialDiffsAndAzimuthCoarseGrids();

  //! Coarse grid initialization of sphere models
  void SphereRadiusCoarseGridInitialization(MCMPointer &mcmUpdateValue,
                                            CostFunctionBasePointer &cost,
                                            MCMType::ListType &workVec,
                                            ParametersType &p);

  //! Coarse grid initialization of NODDI and DDI models
  void ExtraAxonalAndKappaCoarseGridInitialization(
      MCMPointer &mcmUpdateValue, CostFunctionBasePointer &cost,
      MCMType::ListType &workVec, ParametersType &p);

  //! Coarse grid initialization of CHARMED model
  void CHARMEDCoarseGridInitialization(MCMPointer &mcmUpdateValue,
                                       CostFunctionBasePointer &cost,
                                       MCMType::ListType &workVec,
                                       ParametersType &p);

  //! Coarse grid initialization of tensor model
  void TensorCoarseGridInitialization(MCMPointer &mcmUpdateValue,
                                      CostFunctionBasePointer &cost,
                                      MCMType::ListType &workVec,
                                      ParametersType &p);
...
private:
  ...
  //! Utility function to initialize dictionary of sticks for initial sparse
  //! estimation
