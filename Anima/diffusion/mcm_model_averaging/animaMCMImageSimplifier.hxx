...
void MCMImageSimplifier<PixelScalarType>::GenerateOutputInformation() {
...
template <class PixelScalarType>
void MCMImageSimplifier<PixelScalarType>::InitializeReferenceOutputModel() {
  ...
  unsigned int numIsotropicCompartments =
      inImage->GetDescriptionModel()->GetNumberOfIsotropicCompartments();
  for (unsigned int j = 0; j < numIsotropicCompartments; ++j) {
    anima::DiffusionModelCompartmentType compIsoType =
        inImage->GetDescriptionModel()->GetCompartment(j)->GetCompartmentType();
  ...
  if (inImage->GetDescriptionModel()->GetNumberOfCompartments() >
      numIsotropicCompartments)
    compartmentType = inImage->GetDescriptionModel()
                          ->GetCompartment(numIsotropicCompartments)
                          ->GetCompartmentType();
  else
    itkExceptionMacro("Only isotropic compartments, nothing to do here.");

  unsigned int maxCompartments = 0;
  itk::ImageRegionConstIterator<MoseImageType> moseItr(
      m_MoseMap, m_MoseMap->GetLargestPossibleRegion());
...
template <class PixelScalarType>
void MCMImageSimplifier<PixelScalarType>::BeforeThreadedGenerateData() {
...
template <class PixelScalarType>
void MCMImageSimplifier<PixelScalarType>::DynamicThreadedGenerateData(
    const OutputImageRegionType &outputRegionForThread) {
  // Iterator definitions
  typedef itk::ImageRegionIterator<InputImageType> ImageIteratorType;

  InputImageType *inImage = const_cast<InputImageType *>(this->GetInput());
  ImageIteratorType inItr(inImage,
                          this->GetInput()->GetLargestPossibleRegion());
  ImageIteratorType outItr(this->GetOutput(),
                           this->GetInput()->GetLargestPossibleRegion());
  ...
  unsigned int numIsoCompartments =
      outputModel->GetNumberOfIsotropicCompartments();
  ...
  while (!outItr.IsAtEnd()) {
    ...
    for (unsigned int i = 0; i < numIsoCompartments; ++i) {
      ...
      outputModel->GetCompartment(i)->SetCompartmentVector(
          inputModel->GetCompartment(i)->GetCompartmentVector());
    }

    unsigned int pos = numIsoCompartments;
    for (unsigned int i = numIsoCompartments;
         i < outputModel->GetNumberOfCompartments(); ++i) {
      double compWeight = inputModel->GetCompartmentWeight(i);
      if (compWeight <= 0)
        continue;

      weights[pos] = compWeight;
      outputModel->GetCompartment(pos)->SetCompartmentVector(
          inputModel->GetCompartment(i)->GetCompartmentVector());
      ++pos;
    }
