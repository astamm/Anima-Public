...
MCMWeightedAverager::MCMWeightedAverager() {
  ...
  m_InternalSpectralCluster.SetCMeansAverageType(
      SpectralClusterType::CMeansFilterType::Euclidean);
...
void MCMWeightedAverager::SetNumberOfOutputDirectionalCompartments(
    unsigned int val) {
...
void MCMWeightedAverager::ResetNumberOfOutputDirectionalCompartments() {
  m_NumberOfOutputDirectionalCompartments =
      m_OutputModel->GetNumberOfCompartments() -
...
MCMWeightedAverager::MCMPointer &
MCMWeightedAverager::GetUntouchedOutputModel() {
  return m_OutputModel;
}
...
void MCMWeightedAverager::Update() {
  ...
  unsigned int numIsoCompartments =
      m_InputModels[0]->GetNumberOfIsotropicCompartments();
  m_InternalOutputWeights.resize(m_OutputModel->GetNumberOfCompartments());
  std::fill(m_InternalOutputWeights.begin(), m_InternalOutputWeights.end(),
            0.0);
  ...
  for (unsigned int i = 0; i < numIsoCompartments; ++i) {
    ...
    for (unsigned int j = 0; j < numInputs; ++j) {
      ...
      outputLogDiffusivity +=
          m_InputWeights[j] *
          std::log(m_InputModels[j]->GetCompartment(i)->GetAxialDiffusivity());
      outputRadius += m_InputWeights[j] *
                      m_InputModels[j]->GetCompartment(i)->GetTissueRadius();
      sumWeights += m_InputWeights[j];
    }

    if (sumWeights > 0) {
      m_OutputModel->GetCompartment(i)->SetAxialDiffusivity(
          std::exp(outputLogDiffusivity / sumWeights));
      m_OutputModel->GetCompartment(i)->SetTissueRadius(outputRadius /
                                                        sumWeights);
    }
  }

  unsigned int maxNumOutputCompartments =
      m_OutputModel->GetNumberOfCompartments() -
      m_OutputModel->GetNumberOfIsotropicCompartments();
  ...
  for (unsigned int i = 0; i < numInputs; ++i) {
    ...
    for (unsigned int j = numIsoCompartments;
         j < m_InputModels[i]->GetNumberOfCompartments(); ++j) {
  ...
  if (numInputCompartments <= numOutputCompartments) {
    for (unsigned int i = 0; i < numInputCompartments; ++i) {
      ...
      m_InternalOutputWeights[i + numIsoCompartments] =
          m_WorkCompartmentWeights[i];
    }
  ...
  for (unsigned int i = 0; i < numInputCompartments; ++i)
    m_InternalSpectralMemberships[i] =
        m_InternalSpectralCluster.GetClassesMembership(i);
...
void MCMWeightedAverager::ComputeTensorDistanceMatrix() {
  ...
  for (unsigned int i = 0; i < numCompartments; ++i) {
    m_InternalWorkMatrix = m_WorkCompartmentsVector[i]
                               ->GetDiffusionTensor()
                               .GetVnlMatrix()
                               .as_matrix();
    m_leCalculator->GetTensorLogarithm(m_InternalWorkMatrix,
                                       m_InternalWorkMatrix);
    anima::GetVectorRepresentation(m_InternalWorkMatrix,
                                   m_InternalLogTensors[i], 6, true);
  }
  ...
  for (unsigned int i = 0; i < numCompartments; ++i)
    for (unsigned int j = i + 1; j < numCompartments; ++j) {
      ...
      for (unsigned int k = 0; k < 6; ++k)
        distValue += (m_InternalLogTensors[i][k] - m_InternalLogTensors[j][k]) *
                     (m_InternalLogTensors[i][k] - m_InternalLogTensors[j][k]);
...
void MCMWeightedAverager::ComputeOrientationDistanceMatrix() {
  ...
  for (unsigned int i = 0; i < numCompartments; ++i) {
    for (unsigned int j = i + 1; j < numCompartments; ++j) {
      double cosValue = anima::ComputeScalarProduct(m_InternalDDIDirections[i],
                                                    m_InternalDDIDirections[j]);
...
void MCMWeightedAverager::ComputeOutputNonTensorModel() {
  ...
  unsigned int numIsoCompartments =
      this->GetUntouchedOutputModel()->GetNumberOfIsotropicCompartments();
  unsigned int numberOfOutputCompartments =
      m_InternalSpectralMemberships[0].size();

  for (unsigned int i = 0; i < numberOfOutputCompartments; ++i) {
    for (unsigned int j = 0; j < nbOfUsefulFascicles; ++j)
      m_WorkCompartmentWeights[j] =
          referenceDDIWeights[j] * m_InternalSpectralMemberships[j][i];

    anima::DDIAveraging(m_InternalDDINu, m_InternalDDIDiffusivity,
                        m_InternalDDIKappa, m_InternalDDIDirections,
                        m_WorkCompartmentWeights, m_DDIInterpolationMethod,
                        averageNu, averageDiffusivity, averageKappa,
                        averageDirection, averageWeight);

    anima::BaseCompartment *workCompartment =
        this->GetUntouchedOutputModel()->GetCompartment(i + numIsoCompartments);
    ...
    anima::TransformCartesianToSphericalCoordinates(averageDirection,
                                                    averageDirection);
