...
// Types of diffusion model compartments, update if new derived class of base
...
public:
  ...
  virtual double
  GetFourierTransformedDiffusionProfile(double smallDelta, double bigDelta,
  ...
  //! Get compartment overall description vector, mainly for writing, should be
  //! self-contained
  virtual ModelOutputVectorType &GetCompartmentVector() = 0;

  //! Set compartment overall description vector, for setting automatically the
  //! individual parameters when reading from disk
  virtual void
  SetCompartmentVector(ModelOutputVectorType &compartmentVector) = 0;

  //! Copy internal parameters from another compartment, faster than a set/get
  //! compartment vector
  virtual void CopyFromOther(Self *rhs);

  //! Tests equality to another compartment up to a constant (relative and
  //! absolute tolerances)
  virtual bool IsEqual(Self *rhs, double tolerance = 1.0e-2,
                       double absoluteTolerance = 1.0e-8);

  //! Number of parameters to describe the model, these parameters will be
  //! self-contained, i.e. include fixed parameters for example
  ...
  //! Reorient the fascicle compartment using a matrix, the flag specifies if
  //! the transform is affine or rigid
  virtual void Reorient(vnl_matrix<double> &orientationMatrix,
  ...
  virtual double GetOrientationConcentration() {
    return m_OrientationConcentration;
  }
  ...
  virtual void SetOrientationConcentration(double num) {
    m_OrientationConcentration = num;
  }
  virtual void SetExtraAxonalFraction(double num) {
    m_ExtraAxonalFraction = num;
  }
  virtual void SetTissueRadius(double num) { m_TissueRadius = num; }

  itkGetMacro(EstimateTissueRadius, bool);

  double GetPredictedSignal(double smallDelta, double bigDelta,
                            double gradientStrength,
                            const Vector3DType &gradient);

  virtual bool GetTensorCompatible() { return true; }
  //! Get compartment as a 3D tensor (default behavior: throw exception if not
  //! supported by the compartment model)
  ...
  //! Get approximation to parallel diffusivity of the compartment (may be
  //! different from axial diffusivity which is an internal parameter of the
  //! model)
  virtual double GetApparentParallelDiffusivity();

  //! Get approximation to perpendicular diffusivity of the compartment (may be
  //! different from radial diffusivities which are internal parameters of the
  //! model)
...
protected:
  ...
  //! Matrix to hold working value of diffusion tensor approximation to the
  //! model
