...
/**
 * @brief Computes a weighted average of input multi-compartment models. The
 * output model is at the same time giving the number and type of output
 * compartments but also its parameters are erased when performing Update to get
 * the result
 */
...
private:
  ...
  std::vector<double> m_InternalDDIKappa, m_InternalDDINu,
      m_InternalDDIDiffusivity;
