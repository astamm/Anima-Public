...
GetRotationMatrixFromVectors(const VectorType &first_direction,
  ...
  // VectorType tmpVec;
  // tmpVec[0] = first_direction[1] * second_direction[2] - first_direction[2] *
  // second_direction[1]; tmpVec[1] = first_direction[2] * second_direction[0] -
  // first_direction[0] * second_direction[2]; tmpVec[2] = first_direction[0] *
  // second_direction[1] - first_direction[1] * second_direction[0];
  ...
  // itk::SymmetricEigenAnalysis < vnl_matrix <double>, vnl_diag_matrix<double>,
  // vnl_matrix <double> > eigenSystem(dimension+1); vnl_matrix <double>
  // eVec(dimension+1,dimension+1); vnl_diag_matrix <double> eVals(dimension+1);
  ...
  // anima::computeRotationFromQuaternion<double,double>(eVec.get_row(0));
  ...
  itk::Matrix<double, 3, 3> itkRotationMatrix;
  for (unsigned int i = 0; i < 3; ++i)
    for (unsigned int j = 0; j < 3; ++j)
      itkRotationMatrix(i, j) = rotationMatrix(i, j);

  return itkRotationMatrix;
}

template <class ScalarType>
itk::Matrix<double, 3, 3>
GetRotationMatrixFromVectors(const itk::Point<ScalarType> &first_direction,
                             const itk::Point<ScalarType> &second_direction) {
  unsigned int dimension = first_direction.GetPointDimension();
  return GetRotationMatrixFromVectors(first_direction, second_direction,
                                      dimension);
}

template <class ScalarType, unsigned int NDimension>
itk::Matrix<double, 3, 3> GetRotationMatrixFromVectors(
...
void LowerTriangularSolver(vnl_matrix<ScalarType> &matrix, VectorType &rhs,
                           VectorType &result, unsigned int rank) {
...
template <class ScalarType, class VectorType>
void UpperTriangularSolver(const vnl_matrix<ScalarType> &matrix,
